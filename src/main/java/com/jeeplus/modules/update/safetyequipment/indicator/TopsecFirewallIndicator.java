...
public class TopsecFirewallIndicator implements UpdateService {
    ...
    private String uuid="be16a41c89be47cfa070f694e5e782da";
        ...
        synchronized(uuid.intern()) {
            ...
            if(CheckObject.checkList(updateList)){
                for(ResourceIndicatorlist resourceIndicatorlist : updateList){
                    String value="";
                    if("61".equals(resource.getResourceType().getCode())){   //防火墙
                        if("61001".equals(resourceIndicatorlist.getIndicator().getItemType())){
                            value = ConnectionsIndicatorCalculateFirewall.getCurrentConnections(resource.getIp(), resource.getResourceBaseInfo().getRdcommunity(), resource.getResourceBaseInfo().getPort(), resource.getResourceBaseInfo().getDelay(), resource.getResourceBaseInfo().getRepeatnum(), Transformation.null2Integer(resource.getResourceBaseInfo().getManagerType()));
                        }else if("61002".equals(resourceIndicatorlist.getIndicator().getItemType())){
                            value = ConnectionsIndicatorCalculateFirewall.getCps(resource.getIp(), resource.getResourceBaseInfo().getRdcommunity(), resource.getResourceBaseInfo().getPort(), resource.getResourceBaseInfo().getDelay(), resource.getResourceBaseInfo().getRepeatnum(), Transformation.null2Integer(resource.getResourceBaseInfo().getManagerType()));
                        }
                    }
