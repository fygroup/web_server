package com.jeeplus.modules.collect.networkequipment.others.memory;
...
public class MemoryIndicatorCalculateNsfocus {
    private static final String MEMORY_LOAD1 = "1.3.6.1.4.1.19849.2.3.2.0";
    private static final String MEMORY_LOAD2 = "1.3.6.1.4.1.19849.2.8.3.0";
    private static final String MEMORY_LOAD3 = "1.3.6.1.4.1.19849.5.3.2.0";
    ...
    public static String getMemory(String ip, String community, String port, String timeout, String retries, int version) {
        ...
        try{
            ...
            return df.format(p/temList.size());
        }catch (Exception e){
            e.printStackTrace();
        }
        return null;
    ...
    public static String  getMemoryUsedPercent(String ip, String community, String port, String timeout, String retries,int version) {
        ...
        try {
            ResourceOidValue resourceOidValue= SnmpUtil.snmpGet(ip, community, MEMORY_LOAD1, port, timeout, retries, version);
            if(resourceOidValue!=null&&resourceOidValue.getValue()!=null){
                result= df.format(resourceOidValue.getValue());
            }

            if(result==null){
                resourceOidValue= SnmpUtil.snmpGet(ip, community, MEMORY_LOAD2, port, timeout, retries, version);
                if(resourceOidValue!=null&&resourceOidValue.getValue()!=null){
                    result= df.format(resourceOidValue.getValue());
                }
            }
            ...
            if(result==null){
                resourceOidValue= SnmpUtil.snmpGet(ip, community, MEMORY_LOAD3, port, timeout, retries, version);
                if(resourceOidValue!=null&&resourceOidValue.getValue()!=null){
                    result= df.format(resourceOidValue.getValue());
                }
            }
            if(result==null){
                 result= getMemory(ip, community, port, timeout, retries, version);
            }
    ...
    public static void main(String[] args) throws Exception {
        try {
            System.out.println(getMemoryUsedPercent("139.0.1.249", "public","161", "500", "4",1));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
