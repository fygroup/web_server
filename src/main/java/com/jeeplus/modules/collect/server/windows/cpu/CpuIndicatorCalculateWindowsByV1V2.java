package com.jeeplus.modules.collect.server.windows.cpu;
...
public class CpuIndicatorCalculateWindowsByV1V2 {
    ...
    public static List<ResourceOidValue> getCpuUse(String ip, String community, String targetOid, String port, String timeout, String retries, int version) {
        try {
            List<ResourceOidValue> list=SnmpUtil.snmpWalk(ip, community, targetOid, port, timeout, retries, version);
            return list;
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public static Map<String, Object> getMemoryUse(String ip, String community, String port, String timeout, String retries,int version) throws Exception {
        ...
        try {
            ...
            if (response.getResponse() == null) {
                ...
                // responsepdu.getVariableBindings());
                String tem = responsepdu.getVariableBindings().toString();
                used = Transformation.null2Integer(tem.split("=")[1].split("]")[0].trim().toLowerCase());
            }
            if (total == 0) {
                temInt = 0;
            }
    ...
    public static void main(String[] args) throws Exception {
        try {
            ...
            System.out.println(getCpuUse("139.4.48.23", "public","1.3.6.1.2.1.25.3.3.1.2","161", "500", "4",1));
