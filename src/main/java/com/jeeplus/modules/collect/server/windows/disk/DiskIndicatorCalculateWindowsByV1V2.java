package com.jeeplus.modules.collect.server.windows.disk;
...
public class DiskIndicatorCalculateWindowsByV1V2 {
     ...
     *
     */
    public static List<Map<String, Object>> getHardpan(String ip, String community, String port, String timeout, String retries,int version)  {
        ...
        try {
            ...
            for (String label : labelss) {
                ...
                try {
                    ...
                    if ("virtual memory".equals(label.toLowerCase())
                            || "physical memory".equals(label.toLowerCase()) || size.get(i) == 0) {
                        i++;
                        continue;
                    }
                    String usedPercent = "";
                    ...
                    if (label.contains("Label:")) {
                        temMap.put("name", "本地磁盘(" + label.split(":")[0] + ")");
                    } else if (label.contains("新加卷")) {
                        temMap.put("name", "新加卷(" + label.split(":")[0] + ")");
                    } else {
                        i++;
                        continue;
                    }
            ...
            //System.out.println("获取的硬盘信息：" + temList.toString());
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) throws Exception {
        try {
            getHardpan("192.168.1.139", "le","161", "500", "4",1);
