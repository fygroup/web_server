package com.jeeplus.modules.collect.server.linux.memory;
...
public class MemoryIndicatorCalculateLinuxByV1V2 {
    ...
    public static List<Map<String, Object>> getVirtualPhysicalMemory(String ip, String community, String port, String timeout, String retries,int version) {
        ...
        try{
            ...
            if(temList.size()==0){
                ...
                for (String label : labelss) {
                    Map<String, Object> temMap = new HashMap<String, Object>();
                    if ((!("swap space".equals(label.toLowerCase()))&& !("real memory".equals(label.toLowerCase()))) || size.get(i) == 0) {
                        i++;
                        continue;
                    }
                    String usedPercent = "";
                    ...
                    temMap.put("name", label.split(":")[0]);
                    if("swap space".equals((label.split(":")[0]).toLowerCase())) {
                        temMap.put("type", "2");
                    }else {
                        temMap.put("type", "1");
                    }
