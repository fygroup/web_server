...
public class LeaveTestService extends BaseService {
    ...
    public void save(Leave leave, Map<String, Object> variables) {
        ...
        variables.put("type", "leavetest");
        variables.put("busId", businessKey);
        variables.put("title", leave.getReason());//设置标题；
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(ActUtils.PD_LEAVETEST[0], businessKey, variables);
        ...
        logger.debug("start process of {key={}, bkey={}, pid={}, variables={}}", new Object[] {
                ActUtils.PD_LEAVETEST[0], businessKey, processInstance.getId(), variables });
    ...
    public List<Leave> findTodoTasks(String userId) {
        ...
        // 根据当前人的ID查询
        List<Task> todoList = taskService.createTaskQuery().processDefinitionKey(ActUtils.PD_LEAVETEST[0]).taskAssignee(userId).active().orderByTaskPriority().desc().orderByTaskCreateTime().desc().list();
        // 根据当前人未签收的任务
        List<Task> unsignedTasks = taskService.createTaskQuery().processDefinitionKey(ActUtils.PD_LEAVETEST[0]).taskCandidateUser(userId).active().orderByTaskPriority().desc().orderByTaskCreateTime().desc().list();
        // 合并
