...
$(document).ready(function() {
	$('#testNoteTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/test/note/testNote/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jp.confirm('确认要删除该富文本测试记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/test/note/testNote/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/test/note/testNote/form?id="+row.id+"'>"+value+"</a>";
		         }
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#testNoteTable').bootstrapTable("toggleView");
		}
			...
			jp.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/test/note/testNote/import/template';
				  },
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该富文本测试记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/test/note/testNote/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#testNoteTable').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/test/note/testNote/form?id=" + getIdSelections();
  }
