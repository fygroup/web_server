...
$(document).ready(function() {
			...
			,
            {
                ...
                formatter:function(value){
                    if(value == '0,'){
                        ...
                        return 'CPU利用率,CPU利用率';
                    }else{
                        return '通断,CPU利用率,CPU利用率';
					}
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该定时任务记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/monitor/patrol/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#table').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
  ...
  function edit(){
    window.location = "${ctx}/monitor/patrol/form?id=" + getIdSelections();
	}
	// function download(){
     //    // $('#dictValueTable').bootstrapTable("refresh",{query:{dictTypeId:getIdSelections()}})
	// 	window.location = "${ctx}/monitor/patrol/downloadList?id=" + getIdSelections();
	// }
function download(id){
    if(!id){
        id = getIdSelections();
    }
    jp.openDialog('文件下载', "${ctx}/monitor/patrolDocument/list",'800px', '500px');
}
...
//暂停
  function stop(id){
	  ...
	  jp.confirm('确定要暂停任务？', function(){
			jp.loading();  	
			jp.get("${ctx}/monitor/patrol/stop?id=" + id, function(data){
       	  		if(data.success){
       	  			$('#table').bootstrapTable('refresh');
       	  			jp.success(data.msg);
       	  		}else{
  ...
  function resume(id){
	  ...
	  jp.confirm('确定要恢复任务？', function(){
			jp.loading();  	
			jp.get("${ctx}/monitor/patrol/resume?id=" + id, function(data){
       	  		if(data.success){
       	  			$('#table').bootstrapTable('refresh');
       	  			jp.success(data.msg);
       	  		}else{
