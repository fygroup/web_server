...
<html>
<head>
	<title id="titleId">${resource.name}-资源详情</title>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
            ...
            $.ajax({
                type : 'GET',
                url :  "${ctx}/resource/resource/onlineNetInterface",
                ...
                success : function(result) {
                            for(var i=0; i<result.length;i++) {
                                var id = (result[i].name.replace("/", "_"));
                                id = "#" + (id.replace("/", "_"));

                                if ($(id).length > 0) {
                                    $(id).css("background", "#49de70");
                                }
                               }
                },
                error:function(){
		    ...
		    //网络适配器信息展示
			$(".joggle-img").mouseover(function(){
                var listId = $(this).attr("id");
                $("#c" + listId).css("opacity","1");
			});
            $(".joggle-img").mouseout(function(){
                var listId = $(this).attr("id");
                $("#c" + listId).css("opacity","0");
            });

//鼠标右键事件
            $("#interface-first").mousedown(function(e) {
                console.log(e.which);
                //右键为3
                if (3 == e.which) {
                    $(this).find(".interface-first-operation").css({
                        "display": "block"
                    });
                } else if (1 == e.which) {
                    //左键为1
            ...
            $("#interface-first").mouseleave(function(){
                $(this).find(".interface-first-operation").css({
                    "display": "none"
                });
			})
...
<div class="panel panel-primary" style="border:0;">
	...
	<section id="monitor" style="padding-top:0;">
		...
		<div class="fixed-table-body">
			...
			<div class="h-title">
				...
				<div class="pull-right ">
					<%--<select>
						<c:forEach items="${indexTemplateList}" var="item">
		...
		<div class="fixed-table-body" >
			<h4 id="linkNum">连接设备(0)</h4>
			<div id="linkDiv" style="margin-top: 10px;">
			...
			<%--<table  class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
				...
				<tbody>
					<c:forEach items="${networkInterfaceList}" var="networkInterface" varStatus="index">
						<tr>
							<td>${networkInterface.name}</td>
							<td>${networkInterface.capacity}</td>
							&lt;%&ndash;<td></td>
							<td></td>&ndash;%&gt;
							<td>${networkInterface.intRate.rate}</td>
							<td>${networkInterface.outRate.rate}</td>
						&lt;%&ndash;	<td></td>
							<td></td>&ndash;%&gt;
						</tr>
					</c:forEach>
     ...
     <%--   <div class="fixed-table-body">
                ...
                <div class="col-sm-12">
                    <div class="tabs-container">
                        <ul class="nav nav-tabs">
                            <li class=""><a data-toggle="tab" href="#tab-3" aria-expanded="false">承载应用(0)</a>
                            </li>
                        </ul>
		...
		<script>
			$(document).ready(function () {
                $.ajax({
                    type : 'POST',
                    url :  "${ctx}/resource/resource/getnetworkEquipmentLink",
                    data:{resourceId:$("#resourceId").val()},
                    timeout:10*1000,    //超时时间 10s
                    success : function(result) {
                        $("#linkNum").html("<h4 class='table-title pull-left'>连接设备("+result.length+")</h4>");
                        var html="";
                       for(var i=0;i<result.length;i++){
                           var parm="\""+result[i]["resource"].id+"\"";
                           html+= "<div class='link-device-list'>  "+ result[i]["networkInterfaceName"] +"<img class='link-device-img' src='${ctxStatic}/common/images/web-device.png'><a class='link-device-title' style='margin-left: 10px;' href='#' onclick='detail("+parm+")'>"+result[i]["resource"].name+"</a></div>";
                       }
                        $("#linkDiv").html(html);
                    },
                    error:function(){
	...
	<section id="content2" style="padding:0  20px;">
		...
		<div class="row">
			...
			<table class="table information-table" id="informationEditTable">
				<tbody>
				<form:form id="informationForm" action="${ctx}/resourceinformation/resourceInformation/edit"  method="post"  class="form-horizontal">
					...
					<tr>
						...
						<%--<input  name="addedDate" id="informationAddedDate" class="informationClass" value="${resourceInformation.addedDate}" /></td>--%>
					</tr>
					<tr>
						<td class="td-default">维保年限</td><td class="td-hover"><input  name="maintenancePeriod" id="informationMaintenancePeriod" class="informationClass" value="${resourceInformation.maintenancePeriod}"  onafterpaste="if(isNaN(value))execCommand('undo')" /></td>
					...
					<tr>
						...
						<td class="td-default">数据库端口</td><td class="td-hover"><input  name="dbPort" id="informationDbPort" class="informationClass" value="${resourceInformation.dbPort}"  onafterpaste="if(isNaN(value))execCommand('undo')" /></td>
            ...
            function informationEditSave(obj) {
                $.ajax({
                    ...
                    success : function(result) {
                        ...
                        $(".informationClass").attr("readonly","readonly");
						$("#informationEditTable").find("input").toggleClass("editInset");
		...
		<div class="row">
			...
			<table class="table information-table" id="configInfoFormEdit">
				...
				<form:form id="configInfoForm" action="${ctx}/resourceconfiginfo/resourceConfigInfo/edit"  method="post"  class="form-horizontal">
					...
					<tr>
						<td class="td-default">CPU 个数</td><td class="td-hover"><input  name="cpuNum" id="configInfoCpuNum" class="configInfoClass" value="${resourceConfigInfo.cpuNum}"   onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/></td>
						...
						<td class="td-default">单个 CPU 频率</td><td class="td-hover"><input  name="singleCpuRate" id="configInfoSingleCpuRate" class="configInfoClass" value="${resourceConfigInfo.singleCpuRate}" /></td>
					</tr>
					<tr>
						<td class="td-default">单个 CPU 核数</td><td class="td-hover"><input  name="singleCpuCorenum" id="configInfoSingleCpuCorenum" class="configInfoClass" value="${resourceConfigInfo.singleCpuCorenum}"  onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/></td>
						...
						<td class="td-default">内存条个数</td><td class="td-hover"><input  name="memoryNum" id="configInfoMemoryNum" class="configInfoClass" value="${resourceConfigInfo.memoryNum}"  onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/></td>
					</tr>
					<tr>
						...
						<td class="td-default">硬盘个数</td><td class="td-hover"><input  name="diskNum" id="configInfoDiskNum" class="configInfoClass" value="${resourceConfigInfo.diskNum}"  onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/></td>
					...
					<tr>
						...
						<td class="td-default">U数</td><td class="td-hover"><input  name="UNum" id="configInfoUNum" class="configInfoClass" value="${resourceConfigInfo.UNum}" /></td>
					</tr>
					<tr>
						<td class="td-default">控制器数量</td><td class="td-hover"><input  name="controllerNum" id="configInfoControllerNum" class="configInfoClass" value="${resourceConfigInfo.controllerNum}" /></td>
						<td class="td-default">电源模块数量</td><td class="td-hover"><input  name="elepowerModuleNum" id="configInfoElepowerModuleNum" class="configInfoClass" value="${resourceConfigInfo.elepowerModuleNum}" /></td>
					...
					<tr>
						<td class="td-default">已用接口数量</td><td class="td-hover"><input  name="usedInterfaceNum" id="configInfoUsedInterfaceNum" class="configInfoClass" value="${resourceConfigInfo.usedInterfaceNum}" /></td>
						<td class="td-default">可用接口数量</td><td class="td-hover"><input  name="avaliableInterfaceNum" id="configInfoAvaliableInterfaceNum" class="configInfoClass" value="${resourceConfigInfo.avaliableInterfaceNum}" /></td>
						<td class="td-default">特殊板卡数量</td><td class="td-hover"><input  name="specialBoardNum" id="configInfoSpecialBoardNum" class="configInfoClass" value="${resourceConfigInfo.specialBoardNum}" /></td>
					</tr>
					<tr>
		...
		<div class="row">
			...
			<table class="table information-table" id="physicInfoTableEdit">
				<tbody>
            ...
            function physicInfoEditSave(obj) {
                ...
                $.ajax({
                    ...
                    success : function(result) {
                        ...
                        $(".physicInfoClass").attr("readonly","readonly");
                        $("#physicInfoTableEdit").find("input").toggleClass("editInset");
                    },
                    error:function(){
                        alert("保存失败");
                    },
	...
	<section id="content3" style="padding: 0 20px;">
		...
		<div class="row">
			<table id="" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
				...
				<%--<input id="networkInterfaceList" value="${onlineMap}" type="">--%>
				<tbody id="networkInterfaceListView">

					<%--<c:forEach items="${networkInterfaceList}" var="networkInterface" varStatus="index">
						<tr>
							<td>${networkInterface.name}</td>
							<td>${networkInterface.mac}</td>
							<c:if test="${networkInterface.interfaceType  == 6 }"><td>以太网网络接口</td></c:if>
							<c:if test="${networkInterface.interfaceType == 23}"><td>串行网络接口</td></c:if>
							<c:if test="${networkInterface.interfaceType != 6 && networkInterface.interfaceType != 23}"><td></td></c:if>
							<td>${networkInterface.capacity}</td>
							<td>
			...
			<script>
				function initNetworkInterfaceListView() {
                    ...
                    $.ajax({
                        ...
                        success : function(result) {
                            for(var i=0;i<result.length;i++) {
                                html += "<tr><td>" + result[i].name + "</td><td>" + result[i].mac + "</td>";
                                if (result[i].interfaceType !=null && result[i].interfaceType == '6') {
                                    html += "<td>以太网网络接口</td>";
                                } else if (result[i].interfaceType !=null && result[i].interfaceType == '23') {
                                    html += "<td>串行网络接口</td>";
                                } else {
                                    html += "<td></td>";
                                }
                                ...
                                if(result[i].status=='1') {
                                    ...
                                    html += "<td>" + result[i].capacity + "</td><td><div class='text-center'>-</div></td> <td><div class='text-center'>-</div></td></tr>";
                                }
