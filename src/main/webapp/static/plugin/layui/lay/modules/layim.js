...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ...
    ,'{{# } }}'
    ,'</ul>'
    ,'<ul class="layui-unselect layim-tab-content {{# if(!d.base.isfriend && d.base.isgroup){ }}layui-show{{# } }} layim-group">'
      ,'<li>'
      ...
      ,'</li>'
    ,'</ul>'
    ,'<ul class="layui-unselect layim-tab-content  {{# if(!d.base.isfriend && !d.base.isgroup){ }}layui-show{{# } }} layim-history">'
      ,'<li>'
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<span>'+ (content||cache.base.title||'通讯录') +'</span>'
