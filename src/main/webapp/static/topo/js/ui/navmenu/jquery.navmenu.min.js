(function () {
    var keys = null, defaults = null, methods = null, identifiers = null;
    identifiers = {}, keys = {
        name: "jqnavmenu",
        topClass: "navmenu",
        navClass: "nav",
        ...
        close: function (id) {
            if (arguments.length === 1 && id != null) {
                var $this = $(this), options = methods.options.call($this, null), $li = methods.item.call($this, id);
                ...
                return this.each(function () {
                    var $this = $(this), options = methods.options.call($this, null), $ul = $this.children("." + keys.topClass).children("ul." + keys.navClass);
                    options._menu_on = false;
            ...
            return this.each(function () {
                var $this = $(this), $li = methods.item.call($this, id), blockHandler = function (status) {
            ...
            return this.each(function () {
                var $this = $(this), $li = methods.item.call($this, id), config = $li.data(keys.itemConfigKey);
                config.text = text;
            ...
            var $this = this, options = methods.options.call($this, null), index = options._itemElementIndex;
            if (!index) {
            ...
            return this.each(function () {
                var $this = $(this), options = methods.options.call($this, null);
                if (options) {
            ...
            return this.each(function () {
                var $this = $(this), options = $.extend({}, defaults, opts);
            ...
            if (options.autoCloseEvent && options._identifier) {
                ...
                $(document).bind(eventName, $this, function (event, id) {
                    var $this = event.data, options = methods.options.call($this, null);
            ...
            var $this = this, i = null, options = methods.options.call($this, null);
            ...
            $ul.bind("mouseleave", function () {
                var $ul = $(this), $block = $ul.children("." + keys.navBlockClass);
                if (!options._menu_on) {
            ...
            var $this = this, options = methods.options.call($this, null), $li = methods._newItemObject.call($this, menu);
            $ul.append($li);
            $li.bind(keys.openMenuEvent, methods._openNavItem).bind(keys.closeMenuEvent, methods._closeNavItem).bind("mouseenter", function () {
                var $li = $(this), $block = $ul.children("." + keys.navBlockClass).data(keys.blockOverIndexKey, $li.index());
                if ($block.is(":hidden")) {
                    ...
                    $block.css({
                        ...
                        opacity: 0
                    }).show().stop(true, true).animate({opacity: 1}, function () {
                        $(this).css("opacity", 1)
                    })
                } else {
                    var dst = {width: $li.outerWidth(), left: $li.position().left};
                    $li.siblings().each(function () {
                        var $li = $(this), $cover = $li.children("." + keys.coverClass);
                        if ($cover.is(":visible")) {
                            $li.trigger(keys.closeMenuEvent, options);
                            return false
                        }
                ...
                var $li = $(this), $cover = $li.children("." + keys.coverClass), config = $li.data(keys.itemConfigKey);
                if (methods.isDisabled.call($this, $li)) {
                } else {
                    if ($cover.length) {
            ...
            var $this = this, i = null, $ul = $("<ul/>").addClass(keys.menuClass).appendTo($("<div/>").addClass(keys.coverClass).appendTo($li));
            for (i = 0; i < menus.length; i++) {
                if (menus[i].separator) {
            ...
            var $li = null;
            if (menu) {
                var $this = this, icon = menu.disabled ? (menu.disicon || menu.icon) : menu.icon;
            ...
            if ($cover.length && $li.data(keys.childMenuStatusKey) !== "open") {
                ...
                var $menu = $cover.children(), height = $cover.height();
                $menu.css({top: -height - 10});
                $cover.css({height: height + 80, overflow: "hidden"}).show();
                $menu.stop(true, false).delay(cfg.delay).animate({top: 0}, cfg.time, cfg.animate, function () {
            ...
            if ($cover.length && $li.data(keys.childMenuStatusKey) !== "close") {
                ...
                $menu.stop(true, false).delay(cfg.delay).animate({top: -height - 10}, cfg.time, cfg.animate, function () {
            ...
            if ($cover.length && $li.data(keys.childMenuStatusKey) !== "open") {
                ...
                var width = $cover.width(), $menu = $cover.children();
                $menu.css({left: -width - 10});
                $cover.css({width: width + 50, overflow: "hidden"}).show();
                $menu.stop(true, false).delay(cfg.delay).animate({left: 0}, cfg.time, cfg.animate, function () {
            ...
            if ($cover.length && $li.data(keys.childMenuStatusKey) !== "close") {
                ...
                var $menu = $cover.children(), width = $cover.width();
                $cover.css({overflow: "hidden"});
                $menu.stop(true, false).delay(cfg.delay).animate({left: -width - 10}, cfg.time, cfg.animate, function () {
            ...
            var $li = $(this), $icon = $li.children("img." + keys.itemIconClass), cfg = $li.data(keys.itemConfigKey);
            if (status) {
            ...
            var options = methods.options.call(this, null), id = new Date().getTime();
