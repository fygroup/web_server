 ...
 */
(function ($) {
    var defaults = null, publicOptions = null, keys = null, methods = null;
    publicOptions = {monitorSizeTime: 15000, adjustMethodByMonitor: "move"}, defaults = {
        id: "defaultTooltipId",
        title: "",
        content: "",
        skin: "lightblue",
        ...
        get: function () {
            ...
            var $tooltip = this.stop(true, true), options = methods._getOptions($tooltip);
            ...
            if ($.browser.msie && Number($.browser.version.split(".")[0]) < 9) {
                ...
                $tooltip.stop(true, false).css(pos).css({opacity: options.hideOpacity}).delay(options.showAnimateDelay).show(1).animate({opacity: options.showOpacity}, options.showAnimateTime, function () {
                    $tooltip.css({opacity: options.showOpacity});
                    options._status.display = "show";
                    $.isFunction(callback) && callback.call($tooltip, null)
                })
            }
            if (options.isMonitorSize) {
                var monitorTarget = options.isPublicMonitor ? publicOptions : options, fn = monitorTarget._monitor || function () {
                        var callee = arguments.callee, tooltips = callee.tooltips, orders = [];
                        sizes = callee.sizes || (callee.sizes = {}), fn = function () {
                            var callee = arguments.callee, tooltips = callee.tooltips, orders = callee.orders, index = callee.index, $tooltip = tooltips[orders[index]];
                            if ($tooltip && $tooltip.is(":visible") && id) {
                                var size = sizes[id], width = $tooltip.outerWidth(), height = $tooltip.outerHeight();
                                if (size) {
                                    ...
                                    sizes[id] = {width: width, height: height}
                                }
                            }
                            callee.index = index++;
                        ...
                        $.extend(fn, {orders: orders, tooltips: tooltips, index: 0});
                        fn.call(null)
                    };
                if ($.isFunction(fn)) {
            ...
            var $tooltip = this, options = methods._getOptions($tooltip);
            ...
            if ($.browser.msie && Number($.browser.version.split(".")[0]) < 9) {
                ...
                $tooltip.stop(true, false).delay(options.hideAnimateDelay).animate({opacity: options.hideOpacity}, options.hideAnimateTime, function () {
                    $tooltip.css({opacity: options.hideOpacity}).hide(1);
                    options._status.display = "hide"
                })
            }
            if (options._monitor) {
            ...
            if (options._status.display === "showing" || options._status.display === "show" || options._status.display === "moving") {
                var callee = arguments.callee, excute = function (pos) {
            ...
            var $tooltip = this, $content = methods.getContent.call($tooltip, selector);
            ...
            var $tooltip = this, options = methods._getOptions($tooltip), $content = null;
            if (options.isInitBody) {
            ...
            var $tooltip = this, options = methods._getOptions($tooltip), id = methods._getTooltipId(options.id);
            if (options._monitor) {
                clearInterval(options._monitor);
                delete options._monitor
            }
            ...
            options._status = {};
            var id = methods._getTooltipId(options.id), $tooltip = $("<div/>").addClass(keys.tooltipClass).addClass(options.skin).css({
                width: options.width,
                left: options.left,
                top: options.top
            });
            ...
            var $parent = $(options.parent), $container = $parent.children("." + keys.tooltipContainerClass);
            ...
            var options = methods._getOptions($tooltip), method = methods[options.adjustMethodByMonitor] || methods.move;
            method.call($tooltip, {left: options._status.focusLeft, top: options._status.focusTop})
        }, _forceViewport: function (position, $tooltip) {
            var options = methods._getOptions($tooltip), $parent = $(options.parent), fineTuneLeft = $.isFunction(options.offsetLeft) ? options.offsetLeft.call($tooltip, null) : options.offsetLeft, fineTuneRight = $.isFunction(options.offsetRight) ? options.offsetRight.call($tooltip, null) : options.offsetRight, fineTuneTop = $.isFunction(options.offsetTop) ? options.offsetTop.call($tooltip, null) : options.offsetTop, fineTuneBottom = $.isFunction(options.offsetBottom) ? options.offsetBottom.call($tooltip, null) : options.offsetBottom, posLeft = $.isFunction(options.posLeft) ? options.posLeft.call($tooltip, null) : options.posLeft, posTop = $.isFunction(options.posTop) ? options.posTop.call($tooltip, null) : options.posTop, posWidth = ($.isFunction(options.posWidth) ? options.posWidth.call($tooltip, null) : options.posWidth) || $parent.width(), posHeight = ($.isFunction(options.posHeight) ? options.posHeight.call($tooltip, null) : options.posHeight) || $parent.height(), extra1 = null, extra2 = null, tmpTop, tmpLeft, tmpTop2, tmpLeft2, xPosition = "right", yPosition = "below";
            $.extend(options._status, {focusLeft: position.left, focusTop: position.top});
            if (position.left) {
            ...
            $.extend(options._status, {xPosition: xPosition, yPosition: yPosition});
            return position
    ...
    $.tooltip = function () {
        ...
        if (methods[arg0]) {
            var arg1 = arguments[1], $tooltip = methods._getTooltip(arg1);
            if ($tooltip) {
