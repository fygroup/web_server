    ...
    var styles = {
        ...
        init: function (options) {
            ...
            return this.each(function () {
                var $this = $(this), eventNamespace = "jqctxContext-" + (new Date().getTime());
                            ...
                            switch (event.keyCode) {
                                ...
                                case 39:
                                    if ($menu.find("li." + styles.hover + " ul").length > 0) {
                                        $menu.find("li." + styles.hover + ":last").find("ul:first").slideDown(options.animateDelay).offset(forceViewport({top: $menu.find("li." + styles.hover + ":last").offset().top}, $menu.find("li." + styles.hover + ":last").find("ul:first")));
                ...
                $menu.find("li:not(.disabled)").each(function () {
                    $(this).click(function () {
                        ...
                        if (_action !== "") {
                            var _funName = null, _funPara = null;
            ...
            var opts;
            return this.each(function () {
                var $this = $(this), $menu = $this.data("jqctxMenu"), calculatedWidth, $widthTest;
            ...
            function open(opts) {
                var $cover, $sm, $item = $(this), left, top, spare = 20;
                ...
                if (vAdjust) {
                    $cover.css({top: top + $item.outerHeight() - $cover.outerHeight()})
                }
                var hReverse = pos2.left < pos1.left;
                if (hReverse) {
                    left = -$cover.width();
                    $cover.css({left: left});
                    $sm.css({left: $cover.width()}).animate({left: spare}, opts.animateDelay, opts.horizAnimateName, function () {
                        $cover.css({left: left + spare, width: $sm.outerWidth(), overflow: "visible"});
                        $sm.css({left: 0})
                    })
                } else {
                    $sm.css({left: -(left + 5)}).animate({left: 0}, opts.animateDelay, opts.horizAnimateName, function () {
                        $cover.css({width: $sm.outerWidth(), overflow: "visible"})
                    })
                }
            }
            ...
            function close(opts) {
                ...
                $sm.animate({left: left}, opts.animateDelay, opts.horizAnimateName, function () {
                    $cover.hide()
                })
            }
        }, restore: function () {
            return this.each(function () {
                var $this = $(this), $menu = $this.data("jqctxMenu");
            ...
            var pos = forceViewport({top: y, left: x}, $menu, true);
            $menu.offset(pos).css("visibility", visibility);
            if (y > pos.top) {
                var h = $menu.height(), t = pos.top;
                $menu.css({height: 0, top: t + h}).show().animate({height: h, top: t}, defaults.animateDelay)
            } else {
