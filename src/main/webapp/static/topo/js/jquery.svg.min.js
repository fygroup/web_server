(function ($) {
    ...
    $.extend(SVGHandler.prototype, {
        ...
        _registerSVG: function () {
            for (var i = 0, len = document.embeds.length; i < len; i++) {
                ...
                if (!$(container).hasClass($.svg.markerClassName) || $.data(container, PROP_NAME)) {
                    continue
                }
                var svg = null;
                try {
    ...
    $.extend(SVGWrapper.prototype, {
        ...
        style: function (element, styles, settings) {
            ...
            var node = this._makeNode(args.element, "style", $.extend({type: "text/css"}, args.settings || {}));
        ...
        script: function (element, script, type, settings) {
            ...
            var node = this._makeNode(args.element, "script", $.extend({type: args.type || "text/javascript"}, args.settings || {}));
        ...
        defs: function (element, id, settings) {
            ...
            return this._makeNode(args.element, "defs", $.extend((args.id ? {id: args.id} : {}), args.settings || {}))
        },
        ...
        group: function (element, id, settings) {
            var args = this._args(arguments, ["id"], ["id"]);
            return this._makeNode(args.element, "g", $.extend({id: args.id}, args.settings || {}))
        },
        ...
        path: function (element, path, settings) {
            ...
            return this._makeNode(args.element, "path", $.extend({d: (args.path.path ? args.path.path() : args.path)}, args.settings || {}))
        },
        ...
        _poly: function (element, name, points, settings) {
            ...
            return this._makeNode(element, name, $.extend({points: $.trim(ps)}, settings || {}))
        },
        ...
        linearGradient: function (element, id, stops, x1, y1, x2, y2, settings) {
            ...
            var sets = $.extend({id: args.id}, (args.x1 != null ? {
                x1: args.x1,
                y1: args.y1,
                x2: args.x2,
                y2: args.y2
        ...
        merge: function (element, result, refs, settings) {
            ...
            var node = this._makeNode(args.element, "feMerge", $.extend({result: args.result}, args.settings || {}));
            for (var i = 0, len = args.refs.length; i < len; i++) {
                this._makeNode(node, "feMergeNode", {in_: args.refs[i]})
            }
        },
        ...
        specularLighting: function (element, result, in1, surfaceScale, specularConstant, specularExponent, settings) {
            ...
            return this._makeNode(args.element, "feSpecularLighting", $.extend({
                result: args.result,
                in_: args.in1,
        ...
        load: function (url, settings) {
            settings = (typeof settings == "boolean" ? {addTo: settings} : (typeof settings == "function" ? {onLoad: settings} : settings || {}));
            if (!settings.addTo) {
                this.clear(false)
            }
            ...
            if (url.match("<svg")) {
                ...
                $.ajax({
                    url: url, dataType: ($.browser.msie ? "text" : "xml"), success: function (xml) {
    ...
    $.extend(SVGText.prototype, {
        reset: function () {
            ...
            this._parts[this._parts.length] = ["textpath", value, $.extend({href: pathId}, settings || {})];
            return this
        }
    });
