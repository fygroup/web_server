 ...
 * 一些基于jQuery的修补/扩展功能
 *
 * hydra为插件代号, 意为九头蛇怪, 来源希腊语.
 * 九头蛇怪在世界许多文化中均有出现,
 * 如古希腊神话, 中国神话, 日本神话, 波斯古经, 圣经, 非洲传说等
 *
 * @author Zhao Chengjia
 */
(function ($) {
    ...
    $.extend(DateUtil.prototype, {
        ...
        dateFormat: function (date, mask, utc) {
            ...
            var token = /d{1,4}|m{1,4}|yy(?:yy)?|([HhMsT])\1?|[LloZ]|"[^"]*"|'[^']*'/g, timezone = /\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g, timezoneClip = /[^-+\dA-Z]/g;
    ...
    function ProjectUtil() {
        ...
        return {name: pname, path: path, topologyName: topologyName, topologyPath: path + "/" + topologyName}
    }
    ...
    $.extend(StringUtil.prototype, {
        blank: function (str) {
            ...
            var paramsArr = url.substring(url.indexOf("?") + 1).split("#")[0].split("&"), params = {}, key = null, value = null, pair = null;
    ...
    $.detectChromeExtension = function (extensionId, accesibleResource, callback) {
        ...
        $.ajax({
            url: testUrl, timeout: 1000, type: "HEAD", success: function () {
    ...
    $.extend({
        hydraValidateExp: function (exp) {
            ...
            var logicSymbolNum = (exp.split("$$").length - 1) + (exp.split("||").length - 1) + 1, _function = "";
            for (var i = 0; i < logicSymbolNum; i++) {
                var andIndex = exp.indexOf("$$"), orIndex = exp.indexOf("||"), isAnd = andIndex !== -1 && andIndex < orIndex ? true : (andIndex !== -1 && orIndex === -1 ? true : false), expItem = exp.substr(0, andIndex !== -1 || orIndex != -1 ? (isAnd ? andIndex : orIndex) : exp.length), splittedExpItemByOperator = hydraSplitExp(expItem);
    ...
    function hydraTransformExp(variable) {
        var fields = variable.split("."), primary = fields[0].toLowerCase(), _exp = {
                "topology": "$.topology",
    ...
    $.extend(Converter.prototype, {
        requestError: function (XMLHttpRequest, textStatus) {
            ...
            return {0: "请求未发出", 1: "请求发出中", 2: "接收响应内容", 3: "解析响应内容", 4: "响应内容已解析"}[rs] || "未知"
            ...
            if (divided_2_1000 < 0.0001) {
                ...
                if (divided_2_1000 < 0.1 && divided_2_1000 >= 0.0001) {
                    ...
                    if (divided_2_1000 < 100 && divided_2_1000 >= 0.1) {
                        result = this.dight(value / 1000 / 1000, 2);
                        unit = "Mb/秒"
                    } else {
                        if (divided_2_1000 < 100000 && divided_2_1000 >= 100) {
                            result = this.dight(value / 1000 / 1000 / 1000, 2);
                            unit = "Gb/秒"
                        } else {
                            if (divided_2_1000 < 100000000 && divided_2_1000 >= 100000) {
                                ...
                                if (divided_2_1000 < 100000000000 && divided_2_1000 >= 100000000) {
                                    ...
                                    if (divided_2_1000 < 100000000000000 && divided_2_1000 >= 100000000000) {
                                        result = this.dight(value / 1000 / 1000 / 1000 / 1000 / 1000 / 1000, 2);
                                        unit = "Eb/秒"
                                    } else {
                                        if (divided_2_1000 >= 100000000000000) {
                                            result = this.dight(value / 1000 / 1000 / 1000 / 1000 / 1000 / 1000 / 1000, 2);
                                            unit = "Zb/秒"
                                        } else {
                                            return ""
                                        }
            ...
            if (divided_2_1024 < 0.0001) {
                ...
                if (divided_2_1024 < 0.1 && divided_2_1024 >= 0.0001) {
                    ...
                    if (divided_2_1024 < 100 && divided_2_1024 >= 0.1) {
                        result = this.dight(value / 1024 / 1024, 2);
                        unit = "MB"
                    } else {
                        if (divided_2_1024 < 100000 && divided_2_1024 >= 100) {
                            result = this.dight(value / 1024 / 1024 / 1024, 2);
                            unit = "GB"
                        } else {
                            if (divided_2_1024 < 100000000 && divided_2_1024 >= 100000) {
                                ...
                                if (divided_2_1024 < 100000000000 && divided_2_1024 >= 100000000) {
                                    ...
                                    if (divided_2_1024 < 100000000000000 && divided_2_1024 >= 100000000000) {
                                        result = this.dight(value / 1024 / 1024 / 1024 / 1024 / 1024 / 1024, 2);
                                        unit = "EB"
                                    } else {
                                        if (divided_2_1024 >= 100000000000000) {
                                            result = this.dight(value / 1024 / 1024 / 1024 / 1024 / 1024 / 1024 / 1024, 2);
                                            unit = "ZB"
                                        } else {
                                            return ""
                                        }
            ...
            if (result) {
                result = {value: result.value, unit: result.unit + "/秒"}
            }
            return result
        ...
        },/*[{color: "", text: "未知"}, {color: "", text: "正常"}, {
         color: "",
         text: "通知"
         }, {color: "", text: "警告"}, {color: "", text: "主要"}, {color: "", text: "严重"}][level]*/
        alarm: function (level) {
            return [{color: "", text: "未知"}, {color: "", text: "正常"}, {
                color: "",
                text: "提示"
            }, {color: "", text: "一般"}, {color: "", text: "较急"}, {color: "", text: "紧急"}][level]
        }, alarm_disp: function (data, parent, reload) {
            var info = this.alarm(data.value);
            parent.empty();
            if (info) {
                $("<span/>").css("color", info.color).text(info.text).appendTo(parent)
            }
            ...
            if (reload && plugin && plugin.component) {
            } else {
                parent.empty();
                options = $.extend({textSuffix: data.unit}, options);
                $div = $("<div/>").appendTo(parent).percentbar(options);
                plugin = {component: $div};
                parent.data("plugin", plugin)
            }
            plugin.value = data.value;
            var timerName = "converter-delay-timer", timer = parent.data(timerName);
            if (timer) {
                clearTimeout(timer);
            ...
            this.percent2Bar(value, parent, reload, {type: "reverse"})
        }, runtime: function (data, parent, reload) {
            var time = $.date.runtime(data.value), _this = this;
            if (time) {
                var display = function (time) {
                    var nodata = true, counters = parent.data("jscounters"), execute = function (key) {
                        var num = time[key], counter = counters[key].counter, $container = counters[key].container;
                        if (num > 0 || (nodata && key === "ss")) {
                            nodata = false;
                            $container.show();
                            setTimeout(function () {
                                try {
                                    $container.children(".counter").width(8 * (num + "").length);
                ...
                if (reload) {
                    ...
                    var create = function (num, unit, start, key) {
                        var id = _this._unique(key + "-"), counters = parent.data("jscounters");
                        if (!counters) {
                            counters = {};
                        ...
                        if (!$tr || $tr.length === 0) {
                            $tr = $("<tr/>").appendTo($("<table/>").appendTo(parent))
                        }
                        var $container = $("<td/>").addClass("counterContainer").append($("<div/>").addClass("counter").attr("id", id)).append(unit).appendTo($tr);
                        var counter = new JSCounter.NumberCounter(id, {
                            start: start,
                            pretoken: false,
                            comma: false,
                            numberHeight: "12px",
                        ...
                        counter.render();
                        counters[key] = {counter: counter, container: $container}
                    };
                    create.call(this, time.dd, "天", 0, "dd");
                    create.call(this, time.hh, "时", 0, "hh");
            ...
            parent.empty().html(this.text(data.value) + this.unit(data.unit))
        }, integer_disp: function (data, parent, reload) {
            var plugin = parent.data("plugin");
            if (reload && plugin) {
            } else {
                parent.empty();
                var id = this._unique("number-"), $container = $("<div/>").addClass("counterContainer").append($("<div/>").addClass("counter").attr("id", id)).append(this.unit(data.unit)).appendTo(parent), counter = new JSCounter.NumberCounter(id, {
                    start: 0,
                    pretoken: false,
                    comma: false,
                    numberHeight: "12px",
                ...
                counter.render();
                plugin = {container: $container, counter: counter};
                parent.data("plugin", plugin)
            }
            ...
            if (reload && parent.data("plugin") && parent.data("plugin").component && parent.data("plugin").component.feedData) {
                ...
                parent.data("plugin", {component: $div, name: name, percent: percent, options: options, value: value});
                var timerName = "converter-delay-timer", timer = parent.data(timerName);
                if (timer) {
                    clearTimeout(timer);
                    parent.removeData(timerName)
                ...
                parent.data(timerName, setTimeout(function () {
                    parent.removeData(timerName);
                    var plugin = parent.data("plugin"), $div = plugin.component;
                    if ($div.width() > 0) {
                        try {
                            $div.component(plugin.name, plugin.options, plugin.percent ? {percent: plugin.percent} : undefined).feedData("&value=" + plugin.value, 1000)
                        } catch (e) {
                        }
                    } else {
                        parent.data(timerName, setTimeout(arguments.callee, 100))
                    }
                }, 100))
            }
            ...
            this._widget(data.value, parent, reload, "widget_angular_gauge", 0.58, {
                ...
                dials: {dial: [{valuey: "158"}]},
                colorrange: {
                    color: [{minvalue: "0", maxvalue: "50", code: "96C119"}, {
                        minvalue: "50",
                        maxvalue: "80",
                        code: "F7C327"
                    }, {minvalue: "80", maxvalue: "100", code: "FF7460"}]
                }
            })
            ...
            this._widget(data.value, parent, reload, "widget_angular_gauge", 0.58, {
                ...
                dials: {dial: [{valuey: "158"}]},
                colorrange: {
                    color: [{minvalue: "0", maxvalue: "20", code: "FF7460"}, {
                        minvalue: "20",
                        maxvalue: "50",
                        code: "F7C327"
                    }, {minvalue: "50", maxvalue: "100", code: "96C119"}]
                }
            })
        }, temperature: function (data, parent, reload) {
            this._widget(data.value, parent, reload, "widget_thermometer", null, {chart: {numbersuffix: data.unit}})
            ...
            if (reload && plugin) {
                ...
                parent.empty();
                var id = this._unique("percent-"), $container = $("<div/>").addClass("counterContainer lcdFont").append($("<div/>").addClass("counter").attr("id", id)).append(this.unit(data.unit)).appendTo(parent), counter = new JSCounter.NumberCounter(id, {
                    start: 0,
                    pretoken: false,
                    comma: false,
                    numberHeight: "12px",
            ...
            parent.data(timerName, setTimeout(function () {
                parent.removeData(timerName);
                var color = null, plugin = parent.data("plugin"), value = plugin.value;
                if (reverse) {
                    if (value >= 50) {
                        color = "#E54B46"
                    } else {
            ...
            parent.html({"0": "<font color='red'>不支持</font>", "1": "<font color='green'>支持</font>"}[data.value] || "")
        }, avAlgorithm: function (data) {
            return {
                    "1": {"value": "support", "text": "共同支撑", "icon": "images/avalgorithm/support.png"},
                    "2": {"value": "collaboration", "text": "协作完成", "icon": "images/avalgorithm/collaboration.png"},
                    "3": {"value": "composition", "text": "独立组成", "icon": "images/avalgorithm/composition.png"},
                    "4": {"value": "backup", "text": "多机热备", "icon": "images/avalgorithm/backup.png"},
                    "5": {"value": "balancing", "text": "负载均衡", "icon": "images/avalgorithm/balancing.png"}
                }[data] || {"value": "unaffect", "text": "没有依赖", "icon": "images/avalgorithm/unaffect.png"}
            ...
            parent.empty().append($("<span/>").addClass("lcdFont").html(data.value)).append(this.unit(data.unit))
        }, alarmAckStatus: function (data) {
            return {
                    "0": {"text": "告警已全部确认", "icon": "images/icon/alert_history-16.png"},
                    "1": {"text": "含有未确认告警", "icon": "images/icon/alert-16.png"}
                }[data] || null
        }, unit: function (unit) {
            return unit ? "&nbsp;" + unit : ""
        }, _unique: function (prefix) {
            var l = new Date().getTime(), _callee = arguments.callee, _cache = _callee.tmpIdCache;
            if (!_cache) {
