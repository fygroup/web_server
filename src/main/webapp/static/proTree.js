(function($) {
			...
			Tree.prototype.proHTML = function(arr) {
				...
				arr.map(function(item) {
					var lihtml = "<li class='pp'>";
					if(item.child && item.child.length > 0) {

						if(item.pid == 0){
                            lihtml += "<img src='images/upload_success.png' style='display: none;height: 20px;width: 20px' id='0" + item.id + "'><i ischek='true' class='" + self.mouIconOpen + " PP'></i>" +
                                "<span class='pp' id='" + item.id + "'>" + item.name + "</span><input type='hidden' value='" + item.path + "'/>"
						}else{
                            lihtml += "<i ischek='true' class='" + self.mouIconOpen + " PP'></i>" +
                                "<span class='pp' id='" + item.id + "'>" + item.name + "</span>"
						}
